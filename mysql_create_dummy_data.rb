...
class CreateDummyData
  ...
  def is_auto_increment
    lambda do |table_name|
      ret = false
      @target_db.fetch("show create table `#{table_name}`") do |row|
        ai = search_auto_increment[row[:"create table"]]
        pk = search_primary_key[row[:"create table"]]
        pk.each { |key|
          ret = true if ai.include?(key)
        }
      end
      ret
    end
  ...
  def get_primary_key
    lambda { |table_name|
      array = []
      @target_db.fetch("show create table `#{table_name}`") do |row|
        array = search_primary_key[row[:"create table"]]
      end
      array
    }
  end
  ...
  # 廃止予定
  def alter_pk_query
    lambda do |table_name, primary_key, using_hash|
      using_hash_msg = using_hash ? " USING HASH" : ""
      query = "ALTER TABLE `#{table_name}` ADD PRIMARY KEY (#{primary_key.join(', ')})#{using_hash_msg}"
      query
    end
  end

  # 廃止予定
  def search_auto_increment
    lambda do |ddl_str|
      array = []
      ddl_str.each { |line|
  ...
  # 廃止予定
  def search_primary_key
    lambda do |ddl_str|
      array = []
      ddl_str.each { |line|
        if line =~ /PRIMARY KEY/
          group = /.*PRIMARY KEY \((.*?)\).*/.match(line.gsub("`", ""))
          unless group.nil?
            array = group[1].split(",")
          end
        end
  ...
  def drop_index
    lambda do |table_name|
      add_index_quries = []
      queries = @target_db.indexes(table_name).map { |index_name, index_setting|
        msg = index_setting[:unique] ? " UNIQUE" :""
        add_index_quries << "ADD#{msg} INDEX `#{index_name}` (#{index_setting[:columns].join(', ')})"
        "DROP INDEX `#{index_name}`"
      }
      query = return_query = nil
      if queries.size > 0
        query = "ALTER TABLE `#{table_name}` #{queries.join(', ')}"
        return_query = "ALTER TABLE `#{table_name}` #{add_index_quries.join(', ')}"
      end
